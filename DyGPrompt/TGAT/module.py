...
from prompt import structure_prompt_layer,time_prompt_layer
...
class TGAN(torch.nn.Module):
    ...
    def __init__(self, ngh_finder, n_feat, e_feat,
        ...
        input_dim = n_feat.shape[1]
        # self.meta_net_t = nn.Sequential(OrderedDict([
        #     ("linear1", nn.Linear(input_dim, input_dim // 4)),
        #     ("relu", nn.ReLU(inplace=True)),
        #     ("linear2", nn.Linear(input_dim // 4, input_dim))
        # ]))
        self.meta_net_t = META_NET(input_dim=input_dim)
        # self.meta_net = nn.Sequential(OrderedDict([
        #     ("linear1", nn.Linear(input_dim, input_dim // 2)),
        #     ("relu", nn.ReLU(inplace=True)),
        #     ("linear2", nn.Linear(input_dim // 2, input_dim))
        # ]))
        # self._meta_net = nn.Sequential(OrderedDict([
        #     ("linear1", nn.Linear(input_dim, input_dim //4 )),
        #     ("relu", nn.ReLU(inplace=True)),
        #     ("linear2", nn.Linear(input_dim // 4, input_dim))
        # ]))
        self._meta_net = META_NET(input_dim=input_dim)
        self.meta_net_0 = META_NET(input_dim=input_dim)
        ...
        self.structure_prompt = structure_prompt_layer(n_feat.shape[0],n_feat.shape[1])
        self.time_prompt = time_prompt_layer(n_feat.shape[1])
    ...
    def contrast(self, src_idx_l, target_idx_l, background_idx_l, cut_time_l, num_neighbors=20):
        src_embed = self.tem_conv_tt(src_idx_l, cut_time_l, self.num_layers, num_neighbors)
        target_embed = self.tem_conv_tt(target_idx_l, cut_time_l, self.num_layers, num_neighbors)
        background_embed = self.tem_conv_tt(background_idx_l, cut_time_l, self.num_layers, num_neighbors)
    ...
    def contrast_0(self, src_idx_l, target_idx_l, background_idx_l, cut_time_l, num_neighbors=20):
        src_embed = self.tem_conv(src_idx_l, cut_time_l, self.num_layers, num_neighbors)
        target_embed = self.tem_conv(target_idx_l, cut_time_l, self.num_layers, num_neighbors)
        ...
        neg_score = self.affinity_score(src_embed, background_embed).squeeze(dim=-1)
        return pos_score.sigmoid(), neg_score.sigmoid()
    def contrast_tt(self, src_idx_l, target_idx_l, background_idx_l, cut_time_l, num_neighbors=20):
        src_embed = self.tem_conv_tt(src_idx_l, cut_time_l, self.num_layers, num_neighbors)
        target_embed = self.tem_conv_tt(target_idx_l, cut_time_l, self.num_layers, num_neighbors)
        background_embed = self.tem_conv_tt(background_idx_l, cut_time_l, self.num_layers, num_neighbors)
        return src_embed,target_embed,background_embed
    ...
    #temporal_prompt
    def tem_conv_t(self, src_idx_l, cut_time_l, curr_layers, num_neighbors=20):
        assert(curr_layers >= 0)
        ...
        # query node always has the start time -> time span == 0
        if curr_layers==0:
            temp = self.time_encoder(torch.zeros_like(cut_time_l_th))
            src_node_t_embed = self.time_prompt(temp)
        else:
            src_node_t_embed = self.time_encoder(torch.zeros_like(cut_time_l_th))
        ...
        if curr_layers == 0:
            ...
            src_node_conv_feat = self.tem_conv_t(src_idx_l, 
                                           cut_time_l,
                                           curr_layers=curr_layers - 1, 
                                           num_neighbors=num_neighbors)
            ...
            src_ngh_node_batch_flat = src_ngh_node_batch.flatten() #reshape(batch_size, -1)
            src_ngh_t_batch_flat = src_ngh_t_batch.flatten() #reshape(batch_size, -1)  
            src_ngh_node_conv_feat = self.tem_conv_t(src_ngh_node_batch_flat, 
                                                   src_ngh_t_batch_flat,
                                                   curr_layers=curr_layers - 1, 
                                                   num_neighbors=num_neighbors)
            src_ngh_feat = src_ngh_node_conv_feat.view(batch_size, num_neighbors, -1)
    ...
    def tem_conv_s(self, src_idx_l, cut_time_l, curr_layers,num_neighbors=20):
        assert(curr_layers >= 0)
        ...
        src_node_t_embed = self.time_encoder(torch.zeros_like(cut_time_l_th))
        src_node_feat = self.node_raw_embed(src_node_batch_th)
        # src_node_feat = self.structure_prompt(src_node_feat)
        if curr_layers == 0:
            # src_node_feat = self.structure_prompt(src_node_feat)
            return src_node_feat
        else:
            src_node_conv_feat = self.tem_conv_s(src_idx_l, 
                                           cut_time_l,
                                           curr_layers=curr_layers - 1, 
                                           num_neighbors=num_neighbors)
            ...
            src_ngh_node_batch_flat = src_ngh_node_batch.flatten() #reshape(batch_size, -1)
            src_ngh_t_batch_flat = src_ngh_t_batch.flatten() #reshape(batch_size, -1)  
            src_ngh_node_conv_feat = self.tem_conv_s(src_ngh_node_batch_flat, 
                                                   src_ngh_t_batch_flat,
                                                   curr_layers=curr_layers - 1, 
                                                   num_neighbors=num_neighbors)
            src_ngh_feat = src_ngh_node_conv_feat.view(batch_size, num_neighbors, -1)
            ...
            # src_ngn_edge_feat = self.structure_prompt(src_ngn_edge_feat)
            # src_ngn_edge_feat = self.structure_prompt(src_ngn_edge_feat)
            # attention aggregation
            mask = src_ngh_node_batch_th == 0
            attn_m = self.attn_model_list[curr_layers - 1]
            ...
            local, weight = attn_m(src_node_conv_feat, 
                                   ...
                                   mask)
            return local
    def tem_conv_ss(self, src_idx_l, cut_time_l, curr_layers,num_neighbors=20):
        assert(curr_layers >= 0)
        ...
        src_node_t_embed = self.time_encoder(torch.zeros_like(cut_time_l_th))
        src_node_feat = self.node_raw_embed(src_node_batch_th)
        # src_node_feat = self.structure_prompt(flag,id,src_node_feat)
        if curr_layers == 0:
            src_node_feat = self.structure_prompt(src_idx_l,src_node_feat)
            return src_node_feat
        else:
            src_node_conv_feat = self.tem_conv_ss(src_idx_l, 
                                           cut_time_l,
                                           curr_layers=curr_layers - 1, 
                                           num_neighbors=num_neighbors)
            ...
            src_ngh_node_batch_flat = src_ngh_node_batch.flatten() #reshape(batch_size, -1)
            src_ngh_t_batch_flat = src_ngh_t_batch.flatten() #reshape(batch_size, -1)  
            src_ngh_node_conv_feat = self.tem_conv_ss(src_ngh_node_batch_flat, 
                                                   src_ngh_t_batch_flat,
                                                   curr_layers=curr_layers - 1, 
                                                   num_neighbors=num_neighbors)
            src_ngh_feat = src_ngh_node_conv_feat.view(batch_size, num_neighbors, -1)
            ...
            local, weight = attn_m(src_node_conv_feat, 
                                   ...
                                   mask)
            return local
    def tem_conv_tt(self,src_idx_l, cut_time_l, curr_layers,num_neighbors=20):
        assert(curr_layers >= 0)
        ...
        # pai = self.meta_net(src_node_feat)
        ...
        #meta_0
            ...
            Pai = self.meta_net_t(src_node_t_embed.squeeze(1),use_mlp = 2)
            src_node_feat = src_node_feat + Pai
            src_node_t_embed = src_node_t_embed + pai
            
            return src_node_feat
        else:
            src_node_conv_feat = self.tem_conv_tt(src_idx_l, 
                                           cut_time_l,
                                           curr_layers=curr_layers - 1, 
                                           num_neighbors=num_neighbors)
            ...
            src_ngh_node_batch_flat = src_ngh_node_batch.flatten() #reshape(batch_size, -1)
            src_ngh_t_batch_flat = src_ngh_t_batch.flatten() #reshape(batch_size, -1)  
            src_ngh_node_conv_feat = self.tem_conv_tt(src_ngh_node_batch_flat, 
                                                   src_ngh_t_batch_flat,
                                                   curr_layers=curr_layers - 1, 
                                                   num_neighbors=num_neighbors)
            if src_ngh_node_conv_feat.nelement() > 0:
                src_ngh_feat = src_ngh_node_conv_feat.view(batch_size, num_neighbors, -1)
            else:

                src_ngh_feat = torch.zeros(batch_size, num_neighbors, 172).to(device)
            # src_ngh_feat = src_ngh_node_conv_feat.view(batch_size, num_neighbors, -1)
            ...
            src_node_t_embed = self.time_prompt(src_node_t_embed)
            pai = self._meta_net(src_node_conv_feat.unsqueeze(1),use_mlp = curr_layers ) #(N*32)

            Pai = self.meta_net_t(src_node_t_embed.squeeze(1),use_mlp = curr_layers)
            # print(pai.size())
            # print(Pai.size())
            # print(src_node_t_embed.size())
            # print(src_node_conv_feat.size())

            src_node_t_embed=src_node_t_embed+pai
            src_node_conv_feat= src_node_conv_feat+Pai
            # print(pai.shape)
            ...
            # src_node_t_embed = self.time_prompt(src_node_t_embed)
            # pai = self._meta_net(src_node_t_embed.squeeze(1)) #(N*32)
            # # src_node_t_embed[:,:,:32] = pai.unsqueeze(1)
            
            # src_node_conv_feat+=pai
            ...
            local, weight = attn_m(src_node_conv_feat, 
                                   ...
                                   mask)
            return local
    def tem_conv_c(self, src_idx_l, cut_time_l, curr_layers,num_neighbors=20):
        assert(curr_layers >= 0)
        ...
        if curr_layers == 0:
            ...
            src_node_conv_feat = self.tem_conv_c(src_idx_l, 
                                        cut_time_l,
                                        curr_layers=curr_layers - 1, 
                                        num_neighbors=num_neighbors)
            ...
            src_ngh_node_batch_flat = src_ngh_node_batch.flatten() #reshape(batch_size, -1)
            src_ngh_t_batch_flat = src_ngh_t_batch.flatten() #reshape(batch_size, -1)  
            src_ngh_node_conv_feat = self.tem_conv_c(src_ngh_node_batch_flat, 
                                                src_ngh_t_batch_flat,
                                                curr_layers=curr_layers - 1, 
                                                num_neighbors=num_neighbors)
            src_ngh_feat = src_ngh_node_conv_feat.view(batch_size, num_neighbors, -1)
            ...
            local, weight = attn_m(src_node_conv_feat, 
                                ...
                                mask)
            return local
    def tem_conv_no_meta(self,src_idx_l, cut_time_l, curr_layers,num_neighbors=20):
        assert(curr_layers >= 0)
        ...
        #meta_0
            ...
            src_node_conv_feat = self.tem_conv_no_meta(src_idx_l, 
                                           cut_time_l,
                                           curr_layers=curr_layers - 1, 
                                           num_neighbors=num_neighbors)
            ...
            src_ngh_node_batch_flat = src_ngh_node_batch.flatten() #reshape(batch_size, -1)
            src_ngh_t_batch_flat = src_ngh_t_batch.flatten() #reshape(batch_size, -1)  
            src_ngh_node_conv_feat = self.tem_conv_no_meta(src_ngh_node_batch_flat, 
                                                   src_ngh_t_batch_flat,
                                                   curr_layers=curr_layers - 1, 
                                                   num_neighbors=num_neighbors)
            if src_ngh_node_conv_feat.nelement() > 0:
