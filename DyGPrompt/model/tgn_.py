...
class TGN(torch.nn.Module):
  def __init__(self, neighbor_finder, node_features, edge_features, device, n_layers=2,
               ...
               dyrep=False,struc_prompt_tag=False,time_prompt_tag=False,meta_tag=False,tag=1):
    ...
    if self.time_prompt_tag:
      self.time_prompt = time_prompt_layer(input_dim)
    else:
    ...
    # if not struc_prompt_tag:
    self.node_raw_features = torch.from_numpy(node_features.astype(np.float32)).to(device)
    # else:
        # self.node_raw_features = self.struc_prompt([i for i in range(node_features.shape[0])],torch.from_numpy(node_features.astype(np.float32)))
  ...
  def compute_temporal_embeddings(self, source_nodes, destination_nodes, negative_nodes, edge_times,
    ...
    node_embedding = self.embedding_module.compute_embedding(memory=memory,
                                                             ...
                                                             time_diffs=time_diffs,struc_prompt=self.struc_prompt,time_prompt=self.time_prompt,meta_net=self.meta_net,tag=self.tag)
  ...
  def compute_edge_probabilities_P(self, source_nodes, destination_nodes, negative_nodes, edge_times,
                                 edge_idxs, n_neighbors=20):
    """
    Compute probabilities for edges between sources and destination and between sources and
    negatives by first computing temporal embeddings using the TGN encoder and then feeding them
    ...
    source_node_embedding, destination_node_embedding, negative_node_embedding = self.compute_temporal_embeddings(
      source_nodes, destination_nodes, negative_nodes, edge_times, edge_idxs, n_neighbors)
    source_node_embedding = self.prompt(source_node_embedding)
    destination_node_embedding = self.prompt(destination_node_embedding)
    negative_node_embedding = self.prompt(negative_node_embedding)
  ...
  def get_raw_messages(self, source_nodes, source_node_embedding, destination_nodes,
    ...
    if not self.use_destination_embedding_in_message:
      ...
      # source_message = torch.cat([source_memory, destination_node_embedding, edge_features,
      #                             source_time_delta_encoding],
      #                           dim=1)
