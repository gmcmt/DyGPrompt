...
MODEL_SAVE_PATH = f'./ddgcl_models/{args.prefix}-{args.agg_method}-{args.attn_mode}-{args.data}.pth'
...
#loss function
def DebiasedContrastiveLoss(anchor, positive, negatives, temperature=0.5):
...
for epoch in range(NUM_EPOCH):
    ...
    for k in range(num_batch):
        ...
        tgan = tgan.train()
        src_embed, target_embed, background_embed = tgan.contrast_0(src_l_cut, dst_l_cut, dst_l_fake, ts_l_cut, NUM_NEIGHBORS)
        background_embed = background_embed.unsqueeze(1)
