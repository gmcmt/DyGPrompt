...
def get_data_node_classification(dataset_name, use_validation=False):
  ...
  val_time, test_time = list(np.quantile(graph_df.ts, [0.40, 0.45]))
...
def get_dd_data(dataset_name, different_new_nodes_between_val_and_test=False, randomize_features=False):
  ### Load data and train val test split
  graph_df = pd.read_csv('./downstream_data/{}/ds_{}.csv'.format(dataset_name,dataset_name))
  edge_features = np.load('./processed/ml_{}.npy'.format(dataset_name))
  node_features = np.load('./processed/ml_{}_node.npy'.format(dataset_name)) 
...
def get_data(dataset_name, different_new_nodes_between_val_and_test=False, randomize_features=False):
  ...
  val_time, test_time = list(np.quantile(graph_df.ts, [0.70, 0.80]))
  ...
  return node_features, edge_features, full_data, train_data, val_data, test_data, \
         new_node_val_data, new_node_test_data
def get_d_data(dataset_name, different_new_nodes_between_val_and_test=False, randomize_features=False):
  ### Load data and train val test split
  graph_df = pd.read_csv('./downstream_data/{}/ds_{}.csv'.format(dataset_name,dataset_name))
  edge_features = np.load('./processed/ml_{}.npy'.format(dataset_name))
  node_features = np.load('./processed/ml_{}_node.npy'.format(dataset_name)) 
